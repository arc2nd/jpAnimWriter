...
def jpmSaveAnimationToFile(name, min, max, saveType, selected=[], keyableAttrs=[], refReTarget=0):
	...
	for object in selected:
		...
		for attr in keyableAttrs:
			...
			numOfKeys = cmds.selectKey( (object + "." + attr), add=1, k=1, t=(min,max) )
			keyIndices = []
			...
			try:
				for key in keyIndices:
					...
					safeFileCommand = (safeFileCommand + "kv::" + keyValueString + "\n")
			except TypeError:
...
def jpmSavePoseToFile(name, selected=[], keyableAttrs=[], refReTarget=0):
	##initialize some variables
	verboseLevel = 0
	cancel = 0
	safeFileCommand = ""
